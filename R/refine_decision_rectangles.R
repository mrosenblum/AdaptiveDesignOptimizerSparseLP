...
#
   ...
   list.of.rectangles.dec.with.decision.probs <- list.of.rectangles.dec[1:(length(list.of.rectangles.dec)-number_reference_rectangles)]
	...
	for(r_counter in 1:(length(list.of.rectangles.dec)-number_reference_rectangles)){
		r <- list.of.rectangles.dec.with.decision.probs[[r_counter]]
		if(r$preset_decision==1){
			list.of.rectangles.dec.with.decision.probs[[counter_set_split_rectangles]]$d_probs <- r$preset_decision_value
		} else{
      	for(d in decisions){
 	   		if(sum(d==r$allowed_decisions)>0){
			  # use first rectangle in list_of_rectangles_mtp[[d]] as rprime_d in constraint
			  ...
			  variable_start_position <- variable_location(r,d,rprime,rprime$allowed_actions[1])
			  variable_end_position <- variable_location(r,d,rprime,rprime$allowed_actions[length(rprime$allowed_actions)])
  		  	  list.of.rectangles.dec.with.decision.probs[[counter_set_split_rectangles]]$d_probs[[d]] <-sum(z_solution[variable_start_position:variable_end_position])
				} else{list.of.rectangles.dec.with.decision.probs[[counter_set_split_rectangles]]$d_probs[[d]] <- 0}
			}
		}
...
#}}
	...
	for(r in list.of.rectangles.dec.with.decision.probs){
      for(d in r$allowed_decisions){
		  if(!is.null(r$d_probs) &&  r$d_probs[d]>= 0.9){
				list.of.rectangles.dec.with.decision.probs[[counter_set_split_rectangles]]$preset_decision_preliminary <- 1
				list.of.rectangles.dec.with.decision.probs[[counter_set_split_rectangles]]$preset_decision_value_preliminary <- as.numeric(d==c(1,2,3,4))
		  }

		  }
		  if (is.null(r$d_probs) || max(r$d_probs) < 0.9)
		  {
		  	    list.of.rectangles.dec.with.decision.probs[[counter_set_split_rectangles]]$preset_decision_preliminary <- 0
		  	    }
	counter_set_split_rectangles <- counter_set_split_rectangles+1
	}
 ...
 list.of.rectangles.dec.with.decision.probs.augmented <- list()
 for(counter_set_split_rectangles in 1:(length(list.of.rectangles.dec.with.decision.probs)))
 {
	r <- list.of.rectangles.dec.with.decision.probs[[counter_set_split_rectangles]]
	if(r$preset_decision_preliminary == 0 && r$lower_boundaries[1]>=(-4) && r$lower_boundaries[2]>=(-4) && r$upper_boundaries[1]<=4 && r$upper_boundaries[2]<=4){
		...
		for(counter2 in 1:splitx){
			for(counter3 in 1:splity){
				...
				list.of.rectangles.dec.with.decision.probs.augmented <- c(list.of.rectangles.dec.with.decision.probs.augmented,list(r_augmented))
				}
		}
	} else if(r$preset_decision_preliminary == 0){
		r$preset_decision_preliminary <- NULL
		r$preset_decision <- 0
		list.of.rectangles.dec.with.decision.probs.augmented <- c(list.of.rectangles.dec.with.decision.probs.augmented,list(r))
	} else{ ## check if surrounded by same decision rectangles
		surrounded_by_same <- 1
		for(counter2 in 1:(length(list.of.rectangles.dec.with.decision.probs))){
			r2 <- list.of.rectangles.dec.with.decision.probs[[counter2]]
	...
	list.of.rectangles.dec <- list.of.rectangles.dec.with.decision.probs.augmented
}
