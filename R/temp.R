...
#' @export
optimize_design_temp <- function(subpopulation.1.proportion=0.5,
...
#
#system('matlab -nojvm -r "siterprl_power_objective_function_v2" > output_LP_solver')

sln = R.matlab::readMat(paste("sln2M83s_power_objective_function.mat",sep=""))
...
rounding_threshold_H01 <- 1-1e-10 
rounding_threshold_H02 <- 1-1e-10 
rounding_threshold_H0C <- 0.4 #1-1e-10 

while(power_constraint_matrix_H0C[4,] %*% z_integral_components < 0.82)
{
	...
	for(d in r_reference$allowed_decisions){
	  for(rprime in list.of.rectangles.mtp[[d]]){
		...
		if((H01_reject && H02_reject) || sum(action_indicator[c(4,5,6,7)])>1-1e-10 || (sum(action_indicator[c(4,5,6,7)])>rounding_threshold_H0C && (is.null(rprime$upper_neighbor) || sum(upper_neighbor_action_indicator[c(4,5,6,7)])>1-1e-10) && (is.null(rprime$right_neighbor) || sum(right_neighbor_action_indicator[c(4,5,6,7)])>1-1e-10))){# round H0C to 1 and permanently fix rounding
		H0C_reject <- 1
                  deterministic_action <- ifelse((!H01_reject) && (!H02_reject) && (!H0C_reject),1,
                  ifelse((H01_reject) && (!H02_reject) && (!H0C_reject),2,
             	  ifelse((!H01_reject) && (H02_reject) && (!H0C_reject),3,
...
for(d_plot in decisions){
  ...
  plot(0,type="n",xaxt="n",yaxt="n",xlim=c(-9,9),ylim=c(-9,9),main=main_label,xlab=expression(paste(Z[1]^F)),ylab=expression(paste(Z[2]^F)),cex.lab=2, cex.axis=2, cex.main=2, cex.sub=2)
  axis(1,at=seq(-3,3,by=1),labels=-3:3,cex.axis=2)
  axis(2,at=seq(-3,3,by=1),labels=-3:3,cex.axis=2)
	...
	for(d in r_reference$allowed_decisions){
			  for(rprime in list.of.rectangles.mtp[[d]]){
					   ...
					   if(d_plot <=2){
					     rounding_threshold_H01 <- 1-1e-10 #0.7
					     rounding_threshold_H02 <- 1-1e-10 #0.7
					     rounding_threshold_H0C <- 1-1e-10 #0.65
					   }else{
					     rounding_threshold_H01 <- 1-1e-10
					     rounding_threshold_H02 <- 1-1e-10
					     rounding_threshold_H0C <- 1-1e-10
					   }
		...
		if(H0C_reject && !(sum(z_integral_components[variable_start_position+3,variable_start_position+6]>rounding_threshold_H0C))){browser()}
...
if(0==1){
	df1 <- data.frame("NONE")
	addtable2plot(x=-1.25,y=-1.1,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)

	if(d_plot==1){
	  df1 <- data.frame("H02")
	  addtable2plot(x=-2.5,y=2.2,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("H02,\nH0C")
	  addtable2plot(x=0.3,y=1.9,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("ALL")
	  addtable2plot(x=2.2,y=2.4,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("H01,\nH0C")
	  addtable2plot(x=2.2,y=0.3,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("H01")
	  addtable2plot(x=2.2,y=-2.2,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	} else if(d_plot==2){
	  df1 <- data.frame("H02")
	  addtable2plot(x=-2.5,y=2.5,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("H02,\nH0C")
	  addtable2plot(x=0.3,y=1.9,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("ALL")
	  addtable2plot(x=2.2,y=2.4,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("H01,\nH0C")
	  addtable2plot(x=2.2,y=0,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("H01")
	  addtable2plot(x=2.2,y=-2.2,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	}else if(d_plot==3){
	  df1 <- data.frame("ALL")
	  addtable2plot(x=1,y=1,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("H01,\nH0C")
	  addtable2plot(x=2.4,y=0.3,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=1.5)
	  df1 <- data.frame("H01")
	  addtable2plot(x=2.2,y=-1,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	}else if(d_plot==4){
	  df1 <- data.frame("ALL")
	  addtable2plot(x=1,y=1.2,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	  df1 <- data.frame("H02,\nH0C")
	  addtable2plot(x=0.3,y=2.4,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=1.5)
	  df1 <- data.frame("H02")
	  addtable2plot(x=-1,y=2.2,table=df1,bg="white",bty="o",hlines=FALSE,display.colnames=FALSE,cex=2)
	}
	}
